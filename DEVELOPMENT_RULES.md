# DEVELOPMENT_RULES.md — Interactive Guides (MVP)
...
| State | Zustand for all client state. No Redux, no Context for state |
| Data Source | Local JSON files in `src/data/` (static content) |
| Persistence | localStorage for user state (progress, bookmarks, notes) |
| Validation | Zod for runtime validation of JSON content and localStorage data |
| Icons | lucide-react |

### MVP Exclusions

The following are **not** part of the MVP and must not be introduced:
...
The architecture must remain structured so these can be added later by swapping only the infrastructure layer.
...
- **Feature modules are self-contained.** Each feature folder contains its own `components/`, `hooks/`, `store.ts`, `api.ts` (if loading JSON), and `types.ts`.
...
- **Storage access is isolated.** Only `lib/storage.ts` and feature `store.ts` files read/write `localStorage`. Components and hooks never touch `localStorage` directly.
- **JSON content access is isolated.** Only feature `api.ts` files import from `src/data/`. Components never import JSON directly.
...
| Stores | camelCase | `store.ts` within feature |
| Types | camelCase | `types.ts` (feature-level) or `common.ts` (shared) |
...
- **No inline styles.** Use Tailwind classes. If a dynamic value is needed, use `style` with a typed object.
...
- **Stores call api functions or storage helpers.** The store is the orchestrator: component → store action → api/storage → data.
- **Persisted stores hydrate from localStorage on init** and write on every state change. Use `lib/storage.ts` for all read/write.
...
- **All content lives in `src/data/`.** Courses, lessons, quizzes, and learning paths as JSON files.
- **Feature `api.ts` files are the only modules that import JSON.** They return typed data.
- **Validate JSON with Zod** at load time to catch content errors early.

### User State (localStorage)

- **All localStorage access goes through `lib/storage.ts`.** This module provides typed read/write helpers with graceful fallbacks.
- **Keys are namespaced** with `ig:` prefix (e.g., `ig:progress`, `ig:bookmarks`).
- **Handle missing/corrupted data.** Always provide a default value when localStorage read fails or returns invalid JSON.
- **Validate stored data with Zod** before using it, since localStorage can be manually edited.

```typescript
// lib/storage.ts pattern
export function loadState<T>(key: string, schema: z.ZodType<T>, fallback: T): T {
  try {
...
- **No auth guards in MVP.** All routes are public.
...
- **ARIA only when needed.** Prefer semantic HTML over ARIA attributes.
...
- **DRY — Don't Repeat Yourself.** If the same logic appears in 2+ places, extract it.
...
- **Verify before using.** Check `package.json` before importing a library.
- **Minimize dependencies.** Every new dependency must justify its weight.
